;; hs-align-imports.el — Align the import lines in a Haskell file.
...
;;
;; When hs-align-imports is run within the same buffer, the
...
(defun hs-align-imports-collect ()
  ...
  (let ((imports '()))
    (while (not (or (equal (point) (point-max)) (hs-align-imports-after-imports-p)))
      (let ((line (hs-align-imports-line-match-it)))
        (when line
          (let ((match 
                 (hs-align-imports-merge-parts
                  (loop for i from 1 to 8
                        collect (hs-align-imports-chomp (match-string i line))))))
            (setq imports (cons (cons match (line-beginning-position))
                                imports)))))
      (forward-line))
    imports))

(defun hs-align-imports-merge-parts (l)
...
(defun hs-align-imports-chomp (str)
...
(defun hs-align-imports-padding (imports)
...
(defun hs-align-imports-fill (padding line)
...
(defun hs-align-imports-line-match-it ()
...
(defun hs-align-imports-after-imports-p ()
